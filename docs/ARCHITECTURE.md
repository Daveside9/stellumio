...
Stellumio is three planes sharing one vocabulary (the intent):
...
- **Publisher whitelist** — starts at one (Stellumio's publisher). Over
...
Install: `claude mcp add stellumio npx -y @stellumio/mcp@latest`.
...
2. **We never hold user funds.** Payment flows directly from the user's
   Stellar account to the anchor's receiving address. No Stellumio
   account sits between them.
3. **We never touch fiat.** Fiat settlement is between the anchor and the
   user's bank / mobile-money provider. Stellumio has no banking rail.
