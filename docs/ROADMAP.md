...
| **v4 Universal**     | SDK + MCP GA + embeddable widget                      | Planned · scope decomposed post-v3  | `@stellumio/sdk` + `@stellumio/mcp` on npm; 3 reference integrations | ⚪ Not started                    |
...
**Thesis.** Stellumio becomes the default execution layer for any
...
- [ ] **`@stellumio/sdk` on npm** — typed TS client for the HTTP API +
      MCP; React hooks; three reference integrations
- [ ] **`@stellumio/mcp` on npm** — GA MCP server, versioned, with a
      signed CHANGELOG
- [ ] **Embeddable widget** — `<StellumioWidget />` React component +
...
- [ ] ≥3 external projects depending on `@stellumio/sdk` or `@stellumio/mcp`
